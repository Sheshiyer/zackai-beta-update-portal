---
title: Artefacts NotebookLM
description: Hub de recherche pour les rapports, decks, audio, infographies et exports
  structurés générés par NotebookLM.
category: general
tags:
- research
- notebooklm
- artifacts
sources: []
lastUpdated: '2026-03-12'
order: 1
icon: 🧠
---

# Artefacts NotebookLM

Les outputs NotebookLM sont publiés comme une couche de recherche à part entière dans Wave 8. Ils complètent le récit de marque ZackAI par des rapports, decks, infographies, audio et exports structurés directement consultables depuis le wiki.

## Couverture

- **Rapports :** 3
- **Decks :** 4
- **Audio :** 3
- **Infographies :** 3
- **Tables :** 3
- **Flashcards :** 2
- **Quiz :** 2
- **Cartes mentales :** 1

## Rapports

### [More Than a Hug: Why ZackAI is Redefining Screen-Free Play in the Age of AI](/fr/docs/research/report-blog)

"What if the smartest thing in your child’s room felt like a hug instead of a screen?"

- **Page wiki :** [/fr/docs/research/report-blog](/fr/docs/research/report-blog)
- **Artefact brut :** `/notebooklm/report-blog.md`

### [Brand Intelligence Report: ZackAI](/fr/docs/research/report-briefing)

**Mission & Vision** ZackAI is defined by the philosophy of "Soft Intelligence"—the strategic fusion of advanced conversational AI and the tactile sa…

- **Page wiki :** [/fr/docs/research/report-briefing](/fr/docs/research/report-briefing)
- **Artefact brut :** `/notebooklm/report-briefing.md`

### [ZackAI: Comprehensive Study Guide](/fr/docs/research/report-study-guide)

This study guide provides an exhaustive overview of ZackAI, a screen-free AI playtime companion designed for children aged 3–12. It synthesizes the b…

- **Page wiki :** [/fr/docs/research/report-study-guide](/fr/docs/research/report-study-guide)
- **Artefact brut :** `/notebooklm/report-study-guide.md`

## Infographies

### Infographic — Landscape

![Infographic — Landscape](/notebooklm/infographic-landscape.png)

- **Téléchargement :** [infographic-landscape.png](/notebooklm/infographic-landscape.png)

### Infographic — Portrait

![Infographic — Portrait](/notebooklm/infographic-portrait.png)

- **Téléchargement :** [infographic-portrait.png](/notebooklm/infographic-portrait.png)

### Infographic — Square

![Infographic — Square](/notebooklm/infographic-square.png)

- **Téléchargement :** [infographic-square.png](/notebooklm/infographic-square.png)

## Decks

### Detailed Deck — Full

Export NotebookLM prêt à présenter, au format PDF.

- **Téléchargement :** [deck-detailed-full.pdf](/notebooklm/deck-detailed-full.pdf)

### Detailed Deck — Short

Export NotebookLM prêt à présenter, au format PDF.

- **Téléchargement :** [deck-detailed-short.pdf](/notebooklm/deck-detailed-short.pdf)

### Presenter Deck — Full

Export NotebookLM prêt à présenter, au format PDF.

- **Téléchargement :** [deck-presenter-full.pdf](/notebooklm/deck-presenter-full.pdf)

### Presenter Deck — Short

Export NotebookLM prêt à présenter, au format PDF.

- **Téléchargement :** [deck-presenter-short.pdf](/notebooklm/deck-presenter-short.pdf)

## Briefs audio

### Audio Brief — Short

Brief audio généré par NotebookLM pour une revue à l’écoute.

- **Téléchargement :** [audio-brief-short.mp3](/notebooklm/audio-brief-short.mp3)

### Audio Debate

Brief audio généré par NotebookLM pour une revue à l’écoute.

- **Téléchargement :** [audio-debate.mp3](/notebooklm/audio-debate.mp3)

### Audio Deep Dive — Long

Brief audio généré par NotebookLM pour une revue à l’écoute.

- **Téléchargement :** [audio-deep-dive-long.mp3](/notebooklm/audio-deep-dive-long.mp3)

## Tables de données

### Competitive Table

Export CSV avec 5 lignes et 9 colonnes.

- **Lignes :** 5
- **Colonnes :** Brand, Pricing, Target Market, Key Features, Market Position, Strengths, Weaknesses, Unique Differentiators, Source
- **Téléchargement :** [table-competitive.csv](/notebooklm/table-competitive.csv)

### Persona Table

Export CSV avec 8 lignes et 5 colonnes.

- **Lignes :** 8
- **Colonnes :** Attribute Category, Details, Marketing Implications, Content Approach, Source
- **Téléchargement :** [table-persona.csv](/notebooklm/table-persona.csv)

### Product Table

Export CSV avec 8 lignes et 5 colonnes.

- **Lignes :** 8
- **Colonnes :** Feature Name, Description, Customer Benefit, Differentiator, Source
- **Téléchargement :** [table-product.csv](/notebooklm/table-product.csv)

## Flashcards

### Flashcards — Detailed

Deck de flashcards NotebookLM avec 84 cards.

- **Items :** 84
- **Téléchargement :** [flashcards-detailed.json](/notebooklm/flashcards-detailed.json)

### Flashcards — Standard

Deck de flashcards NotebookLM avec 70 cards.

- **Items :** 70
- **Téléchargement :** [flashcards-standard.json](/notebooklm/flashcards-standard.json)

## Quiz

### Quiz — Hard

Jeu de quiz NotebookLM avec 10 questions.

- **Items :** 10
- **Téléchargement :** [quiz-hard.json](/notebooklm/quiz-hard.json)

### Quiz — Medium

Jeu de quiz NotebookLM avec 12 questions.

- **Items :** 12
- **Téléchargement :** [quiz-medium.json](/notebooklm/quiz-medium.json)

## Mind maps

### Mind Map

Export mind map NotebookLM.

- **Téléchargement :** [mind-map.json](/notebooklm/mind-map.json)
